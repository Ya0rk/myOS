...
use crate::drivers::ahci::ahci_driver::libahci::*;
use crate::drivers::ahci::ahci_driver::libata::*;
use crate::drivers::ahci::ahci_driver::platform::*;
use core::ffi::CStr;
...
fn ahci_print_info(ahci_dev: &ahci_device) {
    ...
    ahci_printf!(
        ...
        if cap & HOST_CAP_SNTF != 0 { "sntf " } else { "" },
        ...
        if cap & HOST_CAP_ONLY != 0 { "only " } else { "" },
        if cap & HOST_CAP_PMP != 0 { "pmp " } else { "" },
        if cap & HOST_CAP_FBS != 0 { "fbs " } else { "" },
        if cap & HOST_CAP_PIO_MULTI != 0 { "pio " } else { "" },
        if cap & HOST_CAP_SSC != 0 { "slum " } else { "" },
        if cap & HOST_CAP_PART != 0 { "part " } else { "" },
        if cap & HOST_CAP_CCC != 0 { "ccc " } else { "" },
        if cap & HOST_CAP_EMS != 0 { "ems " } else { "" },
        if cap & HOST_CAP_SXS != 0 { "sxs " } else { "" },
        if cap2 & HOST_CAP2_DESO != 0 { "deso " } else { "" },
        if cap2 & HOST_CAP2_SADM != 0 { "sadm " } else { "" },
        if cap2 & HOST_CAP2_SDS != 0 { "sds " } else { "" },
        if cap2 & HOST_CAP2_APST != 0 { "apst " } else { "" },
        if cap2 & HOST_CAP2_NVMHCI != 0 { "nvmp " } else { "" },
        if cap2 & HOST_CAP2_BOH != 0 { "boh " } else { "" },
    );
}
...
fn ahci_sata_print_info(pdev: &ahci_blk_dev) {
    unsafe {
        ...
        ahci_printf!(
            "S/N: {}\\n",
            CStr::from_ptr(pdev.serial.as_ptr() as *const i8)
                .to_str()
                .unwrap_or("invalid")
        );
        ahci_printf!(
            "Product model number: {}\\n",
            CStr::from_ptr(pdev.product.as_ptr() as *const i8)
                .to_str()
                .unwrap_or("invalid")
        );
        ahci_printf!(
            "Firmware version: {}\\n",
            CStr::from_ptr(pdev.revision.as_ptr() as *const i8)
                .to_str()
                .unwrap_or("invalid")
        );
        ahci_printf!("Capacity: {} sectors\n", pdev.lba);
    }
}
...
fn ahci_host_init(ahci_dev: &mut ahci_device) -> i32 {
    ...
    for i in 0..ahci_dev.n_ports {
        ...
        if timeout <= 0 {
            ahci_printf!("port {} sata link timeout\n", i as u32);
        } else {
            ahci_printf!("port {} sata link up\n", i as u32);
        }
...
fn ahci_port_start(ahci_dev: &mut ahci_device, port: u8) -> i32 {
    ...
    if timeout <= 0 {
        ahci_printf!("ahci port {} failed to start\n", port as u32);
        return -1;
    }
    return 0;
}
