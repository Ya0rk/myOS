...
macro_rules! run_test {
    ($id:ident, $i:literal) => {
        match $id::test() {
            ...
            Err(e) => {
                println!("{} \x1b[31;1mFAILED\x1b[0m, with error info {} catched", stringify!($id), e);
                return Err($i);
            },
        }
    };
...
fn test_expect_fn<T, E>(result: &Result<T, E>, expected: &Result<T, E>, failed_info: &'static str) -> TestResult
where 
    T: PartialEq,
    E: PartialEq
{
...
macro_rules! do_test {
    ($id:ident) => {
        match crate::test::$id::do_test_group() {
            Ok(num) => println!("\x1b[1mTEST GROUP \x1b[0m{} ALL {} TESTS \x1b[32;1mPASSED\x1b[0m", stringify!($id), num),
            Err(num) => {
                println!("\x1b[1mTEST GROUP \x1b[0m{} \x1b[31;1mFAILED\x1b[0m at TEST NO {}", stringify!($id), num);
            },
        }
    };
}
