use crate::{
    fs::{
        ffi::RenameFlags, Dirent, FileMeta, FileTrait, InodeMeta, InodeTrait, InodeType, Kstat, OpenFlags, SEEK_CUR, SEEK_END, SEEK_SET, S_IFBLK, S_IFCHR
    },
    mm::{
        page::Page,
...
impl DevLoopInode {
    pub fn new() -> Arc<dyn InodeTrait> {
        Arc::new(Self {
            ...
            inner: SpinNoIrqLock::new(DevLoopInodeMeta::new(0)),
        })
    }
}
...
impl InodeTrait for DevLoopInode {
    ...
    async fn sync(&self) {
        if let Some(file) = &self.inner.lock().backing_file {
            file.metadata().inode.sync().await;
        }
    }
        ...
        match cmd {
            ...
            LoopIoctl::LOOP_CHANGE_FD => {
                ...
                let old_file_size = meta
                    .backing_file
                    .as_ref()
                    .map(|f| f.metadata().inode.get_size())
                    .unwrap_or(0);
                drop(meta);
                ...
                self.inner.lock().backing_file = Some(file);
                Ok(0)
            }
