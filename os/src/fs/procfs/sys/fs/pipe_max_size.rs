    ...
    pub fn new() -> Arc<dyn InodeTrait> {
        Arc::new(Self {
            metadata: InodeMeta::new(
                InodeType::File,
                0,
                "/proc/sys/fs/pipe-max-size".into(),
            ),
        })
    }
}
...
#[async_trait]
impl InodeTrait for PipeMaxSizeInode {
...
lazy_static! {
    pub static ref PIPE_MAX_SIZE: SpinNoIrqLock<PipeMax> =
        SpinNoIrqLock::new(PipeMax::new("1024"));
}
