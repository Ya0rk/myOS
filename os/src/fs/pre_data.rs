...
//将预加载到内存中的程序写入文件根目录
/// 自动测试
pub async fn autorun() -> Arc<dyn FileTrait> {
    extern "C" {
        fn autorun_start();
        fn autorun_end();
    }

    if let Ok(autorun) = open("/autorun".into(), OpenFlags::O_CREAT) {
        let len = autorun_end as usize - autorun_start as usize;
        let data = unsafe {
            core::slice::from_raw_parts_mut(autorun_start as *mut u8, len) as &'static [u8]
        };
...
pub async fn mbshell() -> Arc<dyn FileTrait> {
    extern "C" {
        fn mbshell_start();
        fn mbshell_end();
    }

    if let Ok(gbshell) = open("/mbshell".into(), OpenFlags::O_CREAT) {
        let len = mbshell_end as usize - mbshell_start as usize;
        let data = unsafe {
            core::slice::from_raw_parts_mut(mbshell_start as *mut u8, len) as &'static [u8]
        };
        info!("data len ={}", data.len());
...
pub async fn gbshell() -> Arc<dyn FileTrait> {
    extern "C" {
        fn gbshell_start();
        fn gbshell_end();
    }

    if let Ok(gbshell) = open("/gbshell".into(), OpenFlags::O_CREAT) {
        let len = gbshell_end as usize - gbshell_start as usize;
        let data = unsafe {
            core::slice::from_raw_parts_mut(gbshell_start as *mut u8, len) as &'static [u8]
        };
        info!("data len ={}", data.len());
        gbshell.write(data).await;
        return gbshell;
...
/// 使用自己的shell
pub async fn initproc() -> Arc<dyn FileTrait> {
    extern "C" {
        fn initproc_start();
        fn initproc_end();
        fn user_shell_start();
    ...
    // user_shell
    if let Ok(user_shell) = open("/user_shell".into(), OpenFlags::O_CREAT) {
        let len = user_shell_end as usize - user_shell_start as usize;
        let data = unsafe {
            core::slice::from_raw_parts_mut(user_shell_start as *mut u8, len) as &'static [u8]
        };
    ...
    // initproc
    if let Ok(initproc) = open("/initproc".into(), OpenFlags::O_CREAT) {
        let len = initproc_end as usize - initproc_start as usize;
        let data = unsafe {
            core::slice::from_raw_parts_mut(initproc_start as *mut u8, len) as &'static [u8]
        };
...
pub async fn test_initproc() -> Arc<dyn FileTrait> {
    extern "C" {
    ...
    // initproc
    if let Ok(test_initproc) = open("/test_initproc".into(), OpenFlags::O_CREAT) {
