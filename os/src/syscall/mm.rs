...
use crate::{
    hal::config::{align_up_by_page, is_aligned_to_page, PAGE_MASK, PAGE_SIZE}, ipc::{shm::{self, ShmAtFlags, ShmGetFlags, ShmObject, ShmidDs, SHARED_MEMORY_MANAGER}, IPCKey, IPCPerm, IPCPermMode}, mm::{memory_space::{vm_area::{MapPerm, VmArea}, MemorySpace, MmapFlags, MmapProt}, user_ptr::user_ref_mut, MapPermission, VirtAddr}, utils::{Errno, SysResult}
};
...
pub fn sys_shmget(key: isize, size: usize, shmflg: i32) -> SysResult<usize> {
    ...
    info!("[sys_shmget] key: {}, size: {}, shmflag: {:?}", key, size, shmflag);
...
pub fn sys_shmctl(shmid: isize, op: isize, buf: *const u8) -> SysResult<usize> {
    info!("[sys_shmctl] shmid: {}, op: {}, buf_addr: {:#x}", shmid, op, buf as usize);
...
pub fn sys_shmat(shmid: isize, shmaddr: *const u8, shmflg: i32) -> SysResult<usize> {
    ...
    info!("[sys_shmat] shmid: {}, shmaddr: {:#x}, shmflag: {:?}", shmid, shmaddr as usize, shmflag);
...
pub fn sys_shmdt(shmaddr: *const u8) -> SysResult<usize> {
    ...
    let shmaddr = VirtAddr::from(shmaddr as usize);
    let task = current_task().unwrap();
    let shmid = task.with_mut_shmid_table(|shmid_table| {
        shmid_table.remove(&shmaddr).ok_or(Errno::EINVAL)
    })?;

    if let Some(shmobj) = SHARED_MEMORY_MANAGER.write().get_mut(&shmid) {

        task.with_mut_memory_space(|m| {
            m.detach_shm(shmaddr)
        });
