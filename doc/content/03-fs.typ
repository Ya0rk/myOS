...
Del0n1x 充分利用 Rust 语言的特性，借鉴 Linux 文件系统设计，以面向 trait的方式进行编程，提供了方便、安全的文件系统接口供内核的其他模块与用户系统调用使用。

Del0n1x 的虚拟文件系统的主要数据结构为`SuperBlock`、`Inode`、`Dentry`、`File`。
...
#h(2em)在文件对象（File）或者目录项对象中会持有数据类型为 `Arc<dyn InodeTrait>` 的 inode 对象。通过 inode 对象实现的`InodeTrait`接口，我们可以获得文件的各种信息，进行文件的读写、创建、删除等操作。在通过`dyn InodeTrait`调用方法时，实际调用的方法将自动分派给对应的文件系统的索引节点（inode）实现。
...
#h(2em)通过`children`字段可以得当前目录项的子目录项，通过`parent`可以获得当前目录项的双亲目录项，注意到这里使用弱引用（不增加引用计数）防止出现循环引用。`inode`字段为所持有的索引节点（inode）。
...
文件对象（file）是进程中已打开文件在内存中的表示。文件由 open 系统调用创建，由 close 系统调用关闭。一个最基本的文件对象由持有的索inode、文件的偏移和其他状态信息组成。用户态程序调用 open 系统调用去创建文件对象时，首先需要根据路径获得对应的目录项，通过目录项获得索引节inode，将索引节点包装在文件对象中，并设置其状态，最后将创建好的文件对象注册到文件描述符表。
...
#h(2em)在 Page Cache 实现中以页对齐的文件偏移（offset）为 key 去获得对应的页。
