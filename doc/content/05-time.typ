...
#h(2em)时间轮推进算法如下所示，通过该函数我们获取到所有超时的定时器waker，返回给上级调用者用于批量唤醒。
// #pagebreak() // 避免跨页
#algorithm-figure(
  pseudocode(
    no-number,
    [*function* advance_to(target_time: Duration) $\to$ Vec<Waker>],
    [wake_list ← empty list],
    [Calculate slots to advance: slots_to_advance = calc_slot(target_time)],
    [*for* each slot to process *do*],
    ind,
    [*for* each timer entry in current slot *do*],
    ind,
    [*if* entry has expired (expire ≤ target_time) *then*],
    ind,
    [Add entry's waker to wake list],
    ...
    [Update time wheel's current time],
    [*if* current time exceeds target time *then* break early],
    ded,
    [*return* wake_list],
  ),
  caption: [时间轮推进算法],
...
我们使用`TimerEntry`数据结构表示定时器，每个定时器都携带专属的`TimerHandle`——一个单调递增的唯一标识符。当异步Future结束其生命周期时，我们需要drop其中剩下的定时器，这时就可以通过对比`TimerHandle`找到对应的定时器。系统先在时间轮中闪电扫描，然后扫描二叉堆。为优化堆内搜索，我们设计了临时缓存策略：将非目标项暂存后重新入堆，避免了重建整个堆的昂贵开销。这种双路径检索确保删除操作始终保持高效。
...
#h(2em)为了充分利用异步的优势，我们将需要监测的任务被封装在一个超时Future中（如下所示），`deadline`表示任务的超时期限，`timer_handle`用于Drop机制中确保定时器资源的回收，即使任务提前完成也不会留下幽灵定时器。我们使用poll轮循的方式对任务进行推进检测，轮询时执行三重检测：首先尝试推进内部任务，其次检查期限是否届满，最后才注册唤醒器，并且唤醒器只会注册一次。
...
#h(2em)目前Del0n1x已实现较为高效的定时任务管理，但是该优化并没有在初赛的测试用例中体现出来，在初赛测试用例中大部分定时器时间为1s，其实都会被分配到最小堆结构中。这也反向说明了对于代码的优化也要在特定的场景中才能得到体现，比如时间轮适合处理大量短周期定时任务，最小堆适合处理少量长周期定时任务。
