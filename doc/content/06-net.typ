...
为了体现Linux中“一切皆文件”的思想，我们为网络系统中的关键结构体`Socket`实现了`Filetrait`，统一了网络和文件系统，方便内核设计中对其进行高效管理。在此基础上，我们对网络模块实现分层架构设计，从下至上依次为物理网络驱动层、传输层、Socket接口层。
...
系统核心围绕`SockMeta`元数据结构和`Socket` trait展开，通过实现`FileTrait`将网络套接字无缝集成到文件系统中，实现了Linux“一切皆文件”的设计哲学。整个架构体现了协议无关性设计，TCP与UDP套接字通过统一的接口向上提供服务，底层差异由各自的具体实现处理。为了利用文件描述符操控套接字，我们将socket映射到进程控制块的`fdtable`中：
#code-figure(
```rust
/// 将一个socket加入到fd表中
pub fn sock_map_fd(
...
#h(2em)`SockMeta`结构体作为套接字的核心元数据容器，采用Rust的强类型系统精确描述套接字状态。其字段设计反映了网络连接的全生命周期：从初始创建时的空端口和端点，到绑定后的本地地址确定，再到连接建立后的远程端点记录。特别是通过Option类型明确区分已初始化和未初始化状态，避免了传统C实现中常见的无效值问题。
...
#h(2em)`Socket` trait定义了完整的套接字操作接口，既包含标准的bind/connect/listen等基本操作，也提供了send_msg/recv_msg等增强功能。关键接口定义如下：
...
```,
    caption: [Socket trait 部分接口定义],
    label-name: "socket-trait",
)

#h(2em)该trait标记为`async_trait`以适应现代异步IO需求，同时继承`FileTrait`实现了文件描述符的统一管理，可以通过`Filetrait`中的`pollin`、`pollout`异步检查网络缓冲区是否可读可写。
// #code-figure(
// ```rust
// #[async_trait]
// pub trait Socket: FileTrait {
...
Del0n1x使用`NetDev`结构体实现网络设备的封装和抽象，其包含两个关键组件：`device`字段表示具体的网络设备类型，当前支持环回接口；`iface`字段维护了访问smoltcp协议栈的通道。这种分离设计使得设备驱动与协议栈保持松耦合，未来扩展新设备类型时无需修改上层协议逻辑。
...
`poll()`方法是整个网络栈的驱动引擎，实现了事件处理的核心循环，每次poll轮循将处理`SOCKET_SET`中所有的句柄。
...
TCP 是一种面向连接的字节流套接字，而UDP 是一种无连接的报文套接字。他们都继承了`SockMeta`中的字段，实现了不同的 `Socket` trait。
...
#h(2em)在Tcp、Udp数据结构中，都是用`SocketHandle`表示自己的句柄。该句柄存放在smoltcp提供的全局`SOCKET_SET`中，该set可以类比为fdtable，方便快速获取到该套接字的相关句柄，并通过该句柄实现与smoltcp底层网络栈功能交互。
...
在网络交互过程中，`PortManager`负责动态管理TCP/UDP端口资源的分配与回收。该系统采用多层级管理策略，在保证线程安全的前提下实现高效端口分配。为了记录端口分配清空，Del0n1x采用了双位图的设计，单个协议仅需8KB内存（65536位）即可实现O(1)复杂度的状态查询，相比传统哈希表节省了90%以上的内存开销。双位图设计彻底避免了TCP/UDP端口冲突。
