...
#h(2em)需要注意的是，在RISC-V64架构下，我们通过检测fs寄存器是否被使用决定是否保存浮点寄存器，这样的设计减少上下文切换的开销。但是通过查阅手册，LoongArch64架构中并没有提供这样的寄存器，所以我们将LoongArch64中浮点寄存器的保存处理放置在`__trap_from_user`，同时我们在TrapContext中增加了`fcsr`字段用于保存浮点控制状态寄存器。`fcsr`是浮点运算单元（FPU）的核心控制寄存器，它负责管理浮点运算的异常标志、舍入模式、使能控制等关键功能。

== 处理过程

=== 内核中断异常处理

以下均以RISC-V64为例。Del0n1x的内核态中断异常处理目前支持时钟中断和地址缺页异常（对于外部中断放置于比赛下一阶段进行完善） ，代码如下：
