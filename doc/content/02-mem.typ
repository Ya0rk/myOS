...
//
懒分配（Lazy Allocation）机制将进程堆栈、mmap匿名映射等区域分配内存页的时机延迟到访问时。Del0n1x 会为每个进程创建一个8 MB大小的用户栈，分配全部8 MB的内存页会带来不可忽视的时空开销。应用懒分配机制后，用户栈分配的时间开销分散在运行时，同时未被使用的栈空间将永远不会分配。同理，mmap匿名映射也应用懒分配机制降低时空开销。
