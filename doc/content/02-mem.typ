...
由于 RISC-V 架构下只有一种地址翻译模式，也就只有一个虚拟地址到物理地址的直接映射，因此令两个trait的实现完全相同即可。
...
#figure(
    ...
    caption: [进程缺页异常总览（排版有问题）],
    supplement: [图]
)

=== 写时复制（Copy on Write，CoW）

当有多个进程可写访问同一内存资源（如文件缓存、物理内存页）但又不希望在进程间共享修改时，CoW 机制能推迟复制操作的发生，直到任意一方尝试进行修改操作。 CoW 机制能显著提升操作系统的时空效率。Del0n1x 依赖缺页异常处理实现了对 CoW 机制的支持，并全面使用 CoW 机制以保证进程`fork`、`mmap`文件私有映射等功能的高效。
...
//
懒分配机制将进程堆栈、mmap匿名映射等区域分配内存页的时机延迟到访问时。Del0n1x 会为每个进程创建一个`8 MB`大小的用户栈，分配全部`8 MB`的内存页会带来不可忽视的时空开销。应用懒分配机制后，用户栈分配的时间开销分散在运行时，同时未被使用的栈空间将永远不会分配。同理，mmap匿名映射也应用懒分配机制降低时空开销。
